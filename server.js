...
app.get("/logs", (request, response) => {
  ...
  http.get(`https://www.goodreads.com/book/title?id=${query}`, httpResponse => {
    ...
    httpResponse.on('end', () => {
      ...
      if (imgTagMatch == null) {
        ...
        const url = imgTag.match(/(src=".*")/g)[0].replace('src="', '').replace('"', '')
        result.coverUrl = url
      }
      ...
      if (authorTagMatch == null) {
        ...
        const author = authorTag.match(/(<span itemprop="name">.*<\/span>)/g)[0].replace('<span itemprop="name">', '').replace('<\/span>', '')
